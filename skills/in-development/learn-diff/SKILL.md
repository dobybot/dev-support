...
2. ลงทะเบียน run:
   ...
   **`--title` (และ `title` ใน `run.json`) ห้ามขึ้นต้นด้วย `PR #<N>`** — viewer ประกอบ
   `PR #<N> — ` ให้เองทั้งหัวเรื่อง, topbar และชื่อแท็บ · ใส่มาเองได้ "PR #280 — PR #280 — …"
   (viewer strip prefix ที่เลขตรงกันทิ้งให้อยู่แล้ว แต่อย่าพึ่งมัน) — เขียนแค่ชื่อเรื่องล้วน ๆ
