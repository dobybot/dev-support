...
- **coverage view ไม่เขียนอะไรลง checklist ตรวจรับ** — มัน annotate หน้า verify เฉย ๆ กฎ PD-by-default
  ...
  ปฏิเสธ ไม่ใช่เลื่อน
## Aug 7, 2026 — #47: polish batch จาก feedback ของ run จริง (#39, #41, #42, #46)

feedback ทั้งชุดมาจากการอ่าน run `pr-280-pos-qr-member-code` จริง — สี่จุดเล็กที่รวมกัน
แล้วกินสมาธิผู้อ่านทุกครั้งที่เจอ

- **#39 — window scroll หายตอนออกจากเต็มหน้าจอ**: ต้นเหตุคือ full-screen ซ่อนคอลัมน์เนื้อหา
  ด้วย CSS (`hidden`) เอกสารจึงหดจน browser clamp `scrollY` เป็น 0 ให้เอง — พอกลับมาเนื้อหา
  ก็อยู่บนสุด · แก้ด้วย `pageScrollTransition()` ใน `src/lib/reading-panel.ts` (ฟังก์ชันล้วน
  เทสต์ที่ `test/reading-panel.test.ts`) ที่ hook เรียกจาก **ทางออกทั้งสามทาง** (ปุ่ม toggle,
  Esc ซึ่งวิ่งผ่าน `toggleFullscreen` อยู่แล้ว, และ `close` ขณะเต็มหน้าจอ) แล้วคืนค่าใน
  `useLayoutEffect` — ต้องเป็น layout effect ไม่ใช่ effect ปกติ เพราะคอลัมน์เนื้อหาต้องกลับมา
  ก่อนเอกสารจึงสูงพอให้ scroll กลับได้จริง · **ปิด panel ตอนไม่ได้เต็มหน้าจอไม่คืนอะไร** (window
  scroll ไม่เคยถูกแตะ การกระโดดจึงเป็นการทำลายตำแหน่งที่ผู้อ่านอยู่จริง) · เข้าเต็มหน้าจอซ้ำ
  ไม่ทับค่าที่จำไว้ — ตอนนั้น scrollY เป็น 0 ไปแล้ว ทับ = ตำแหน่งเดิมหาย (บทเรียนเดียวกับ #18:
  เก็บค่าดิบ อย่าเก็บค่าที่สภาพแวดล้อมชั่วคราวบิดไปแล้ว)
- **side effect ห้ามอยู่ใน state updater**: `toggleFullscreen` อ่าน `fullscreen` จาก closure
  แทนที่จะคิดใน `setFullscreen((v) => …)` — แอปรันใต้ `StrictMode` ซึ่งเรียก updater สองรอบ
  การจำ/คืน scroll ที่อยู่ในนั้นจะทำงานสองครั้ง
- **#42 — prefix ซ้ำแก้ฝั่งอ่าน ไม่ใช่ฝั่งเขียนอย่างเดียว**: `src/lib/pr-title.ts` (ตามแบบ
  `strip-duplicate-h1.ts`) เป็นที่เดียวที่ประกอบ `PR #N — ` — `runHeading()` /
  `runDocumentTitle()` ใช้ร่วมกันทั้ง heading, topbar และชื่อแท็บ · strip เฉพาะ **เลขที่ตรงกับ
  `pr.number`** เท่านั้น (เลขอื่นคือข้อมูลของผู้เขียน ตัดทิ้ง = กลืนข้อมูล) รับ separator
  `— – - :` และซ้อนหลายชั้น · แก้ฝั่งอ่านเพราะ run เก่าที่ generate ไว้แล้วต้องแสดงถูกทันที
  โดยไม่ต้อง regenerate — ฝั่งเขียนกำชับใน `SKILL.md` (ขั้นตอนลงทะเบียน) และ
  `references/content-format.md` อีกชั้น
- **#41 — document.title ตั้งจาก `RunLayout`** พร้อม cleanup กลับเป็น `APP_TITLE`
  (ต้องตรงกับ `<title>` ใน `index.html`) — คำนวณเป็น string ก่อนเข้า effect ไม่ใช่ผูกกับ object
  `data` เพราะ SSE reload ทุกครั้งจะสั่งตั้งชื่อใหม่โดยไม่จำเป็น
- **#46 — พับ sidebar แล้ว "หายทั้งแถบ" ได้เพราะย้าย navigation ขึ้น topbar ก่อน**: rail `w-8`
  ของ #27 มีอยู่เพราะปุ่มกางกลับกับลิงก์ "← run ทั้งหมด" ไม่มีที่อยู่อื่น — ตั๋วนี้เพิ่ม topbar
  บาง sticky (h-11, z-30) รับสองตัวนั้น + ชื่อ PR ย่อ (บอกว่าอยู่ run ไหนตอน scroll ลึก) แล้ว
  `aside` จึงไม่ render เลยตอนพับ เนื้อหา reflow เต็ม · `aside` เปลี่ยนเป็น `sticky top-14`
  ให้พ้น topbar · **ปุ่ม toggle เป็น `hidden lg:block`** — จอที่แคบกว่านั้น sidebar ซ่อนอยู่แล้ว
  ปุ่มที่กดแล้วไม่มีอะไรเกิดคือ dead click (หลักข้อ 9) · key `learn-diff:nav-collapsed` เดิม
  ไม่เปลี่ยน — คนที่พับไว้แล้วไม่ต้องพับใหม่
- **Test seam เดิมพอ** ตาม Testing Decisions ของสเปก: pure logic สองก้อน (`pr-title`,
  `pageScrollTransition`) เทสต์ผ่าน vitest ระดับ node · topbar/reflow/ชื่อแท็บจริงตรวจด้วยมือ
  (ตรวจแล้วครบทั้ง 3 ทางออกของ full-screen, การพับ, และชื่อแท็บตอนออกจากหน้า run)

## Aug 7, 2026 — #48: touch support — pan/pinch ไดอะแกรม + กดค้างเปิดเมนู navigation (#40, #43)

feedback มาจากการอ่าน run `pr-280-pos-qr-member-code` บนมือถือผ่าน tunnel: ไดอะแกรมใหญ่ดูได้
แค่ส่วนที่พอดีจอ และ navigation ทั้งชุด (F12 / Shift+F12 / Alt+F12 / Cmd-click) ไม่มีทางเรียกเลย
เพราะไม่มีคีย์บอร์ด

- **pointer events ชุดเดียวครอบทั้งเมาส์และนิ้ว ไม่เขียน mouse\* + touch\* สองชุด** — สองชุด
  แปลว่ามีสองที่ให้พังคนละแบบ · pan/zoom เป็น **CSS transform บน wrapper ของ SVG**
  (`.ld-viewport-content`) ไม่ใช่การแก้ SVG หรือ viewBox — `src/lib/diagram/` จึงไม่ต้องรู้เรื่องนี้
  เลย (มันเป็นเรื่องของ "กล่องที่มอง" ไม่ใช่ของ engine ที่วาด) กำแพง mermaid เดิมยังอยู่ครบ
- **คณิตแยกเป็นฟังก์ชันล้วน** ที่ `src/lib/pan-zoom.ts` (เทสต์ `test/pan-zoom.test.ts`) ·
  หลักที่ยึด: (1) `zoomAt()` ตรึงจุดใต้นิ้วไว้กับที่ และคำนวณจาก scale **หลัง clamp** เสมอ
  ไม่งั้นพอชนขอบซูมภาพจะไหลออกจากนิ้วทีละนิด (2) `pinchViewport()` คิดจาก viewport **ตอนเริ่ม
  หนีบ** ทุกเฟรม ไม่สะสมทีละเฟรม — สะสมแล้วเศษปัดจะพอกจนปล่อยนิ้วแล้วภาพไม่กลับที่เดิม
  (บทเรียนเดียวกับ #18: เก็บค่าดิบ อย่าเก็บค่าที่ถูกบิดระหว่างทาง)
- **transform เขียนลง DOM ตรง ๆ ระหว่างลาก ไม่ผ่าน React state** (`src/lib/use-pan-zoom.ts`) —
  pointermove ยิง 60+ ครั้ง/วินาที การ re-render ทุกเฟรมลาก panel โค้ดที่เปิดอยู่ไปหน่วงด้วย ·
  state ถูก sync ตอนจบ gesture เท่านั้น (ปุ่มรีเซ็ต/ตัวเลข %)
- **threshold แยก tap ออกจาก drag คือสิ่งเดียวที่กัน node click เดิมหาย** — ขยับไม่เกิน 6px ยัง
  ไม่ขยับภาพเลย และ click ที่ตามหลังการลากถูกกลืนตอน capture · **บั๊กที่เจอตอนทดสอบด้วยมือ**:
  ธง `dragged` ค้างหลังหนีบ ทำให้ *คลิกถัดไป* ในกล่องโดนกลืน — เห็นชัดที่ปุ่มซูม (กดหลังหนีบแล้ว
  ไม่เกิดอะไร = dead click ตรง ๆ) เพราะปุ่มไม่ได้เริ่มด้วย pointerdown ที่ล้างธง · แก้ด้วยการ
  ยกเว้น `.ld-viewport-controls` ทั้งขา pointerdown และขา click
- **`touch-action: none` จำกัดอยู่แค่กล่องไดอะแกรม** และกล่องสูงไม่เกิน `75vh` **เฉพาะ
  `@media (pointer: coarse)`** — บนมือถือถ้าไดอะแกรมสูงเต็มหน้า นิ้วจะไม่เหลือที่ว่างให้เลื่อน
  หน้าเลย กลายเป็นหลุมดัก (user story 5) · **แต่เหตุผลนั้นใช้ได้กับนิ้วเท่านั้น** — รอบแรกเผลอ
  cap ทุกอุปกรณ์ ทำให้ไดอะแกรมแนวตั้ง (ซึ่ง learn-diff generate เป็นปกติ) ถูกตัดบน desktop ทั้งที่
  เดิมเห็นเต็ม และ `overflow: hidden` ไม่มี scrollbar บอกด้วยซ้ำว่ายังมีต่อ: เป็นการเปลี่ยน
  พฤติกรรมของ surface หลักโดยไม่มีใครขอ (สเปกขอแค่ "เพิ่ม pan/pinch") ·
  ตอน render ไม่สำเร็จกล่องกลับเป็น `overflow-x-auto` ธรรมดา — ไม่มีเหตุผลให้กิน gesture ทับ
  source ยาว ๆ ที่แสดงแทนรูป
- **พิกัดของ event ต้องแปลงเข้าระบบพิกัดของคณิตก่อน ไม่ใช่ส่งระยะจากขอบกล่องเข้าไปดิบ ๆ** —
  `pan-zoom.ts` ยึดสูตร `screen = x + s·c` โดย `screen` วัดจาก **จุดกำเนิดของ
  `.ld-viewport-content`** แต่กล่องมี `p-4` อยู่ จุดกำเนิดจริงจึงห่างจากขอบกล่อง 16px:
  ส่งพิกัดดิบเข้าไปแล้วการซูมทุกทางคลาด `P·(1−r)` px (หนีบ 2× = เลื่อน 16px, ซูมถึงเพดาน 4× =
  48px) ส่วน pan ไม่รู้สึกเลยเพราะเป็น delta ล้วน · **เทสต์รอบแรกผ่านทั้งที่ของจริงเพี้ยน**
  เพราะพิสูจน์ invariant ในระบบพิกัดของตัวฟังก์ชันเอง (`screenPointOf`/`contentPointAt` ใช้สูตร
  เดียวกับ `zoomAt`) — บทเรียน: การต่อสองระบบพิกัดคือจุดที่ต้องมีเทสต์ ไม่ใช่ตัวคณิตที่ปิดในตัวเอง
  อยู่แล้ว · แก้ด้วย `contentOrigin()` ที่ **วัด padding จาก rect จริง** (scale ไม่ขยับขอบซ้าย/บน
  เพราะ `transform-origin: 0 0` ระยะที่เหลือหลังหักการเลื่อนคือ padding พอดี) ไม่ hardcode 16
- **wheel เปล่าไม่ซูม** ต้องกด ctrl/cmd (trackpad ของ mac ส่ง `ctrlKey` มาให้เองตอนหนีบ) —
  wheel เปล่าปล่อยผ่านให้หน้าเลื่อนตามปกติ ด้วยเหตุผลเดียวกับข้อบน
- **กดค้าง (#43) อยู่ใน extension navigation เดิม** (`src/lib/code/navigation.ts` แหล่งเดียวกับ
  mousedown ของ Cmd-click) และทุกคำสั่งในเมนูลงท้ายที่ `dispatchNav()` ตัวเดียวกับ F12 —
  ไม่มีเส้นทาง logic ที่สองหลังจุดนั้น · keymap กับ modifier-click เดิมไม่ถูกแตะเลย
- **state machine ของการกดค้างเป็นฟังก์ชันล้วน** ที่ `src/lib/code/long-press.ts` (เทสต์
  `test/long-press.test.ts`) กฎที่ล็อกไว้คือ **การยกเลิกต้องง่ายกว่าการติด**: ปล่อยก่อนเวลา /
  ขยับเกิน 10px (วัดจากจุดเริ่มกดเสมอ ไม่ใช่จุดล่าสุด ไม่งั้นลากช้า ๆ จะไม่มีวันยกเลิก) /
  มีนิ้วที่สองลงมา = ยกเลิกทั้งหมด — ทางที่ผู้ใช้เดินทุกวัน (แตะ, ลากเลือกข้อความ, หนีบซูม)
  ต้องไม่ถูกเมนูเด้งขวาง (user story 7)
- **กดค้างรับเฉพาะ pointerType `touch`/`pen` ไม่รับเมาส์** — บน desktop การกดค้างโดยไม่ขยับคือ
  จังหวะปกติของการเริ่มลากเลือกข้อความ และ F12/Cmd-click ก็ครบอยู่แล้ว เมนูที่เด้งขวางของเดิม
  คือ regression (user story 10) · โค้ดยังเป็น pointer event ชุดเดียวตามสเปก ต่างกันแค่เงื่อนไข
  ที่ยอมให้ติด
- **กดค้างบนที่ที่ไม่ใช่ identifier ไม่เปิดเมนู** — เมนูที่ทุกคำสั่งทำอะไรไม่ได้แย่กว่าไม่มีเมนู
  และการกดค้างธรรมดา (เลือกข้อความ) ต้องยังทำงาน · ถ้าเลือกคำสั่งแล้ว `dispatchNav` คืน false
  (เอกสารเปลี่ยนไประหว่างเมนูเปิดค้าง) เมนูขึ้นข้อความบอกแล้วค่อยปิดเอง ไม่ปิดเงียบ ๆ (หลักข้อ 9)
- **เมนูเป็น DOM ล้วนใน `document.body`** (`src/lib/code/nav-menu.ts`) ไม่ใช่ React component:
  คนตัดสินใจเปิดคือ extension ที่อยู่หลังกำแพง การส่งสัญญาณข้ามไปฝั่ง React แล้วส่งพิกัดกลับมา
  เป็นทางอ้อมที่ยาวกว่าและมีสถานะให้พังมากกว่า · ต้องอยู่ใน body เพราะกล่องโค้ดมี overflow ของ
  ตัวเอง · **มี grace period 300ms หลังเปิด**: นิ้วที่เปิดเมนูยังไม่ยกขึ้น พอยกแล้ว click ที่ตามมา
  จะตกใส่ปุ่มที่เพิ่งโผล่มาพอดี ถ้าไม่กันไว้เมนูจะเลือกข้อแรกให้เอง (และกลืน click/contextmenu
  ที่ตามหลังการกดค้างอีก 700ms)
- **Test seam เดิมพอ** ตาม Testing Decisions ของสเปก: pure logic สองก้อน (pan/pinch, long-press)
  เทสต์ผ่าน vitest ระดับ node · การผูก pointer event จริงตรวจด้วยมือ — รอบนี้ตรวจบน browser จริง
  ด้วย run ตัวอย่าง (ลาก/หนีบ/แตะ node/ปุ่มซูม/รีเซ็ต และกดค้าง → definition, references, peek
  ครบทั้งสาม) ที่ยังเหลือให้ตรวจบนมือถือจริงผ่าน tunnel คือความรู้สึกของ gesture กับขนาดปุ่ม

## Aug 7, 2026 — #49: comment ขึ้น GitHub จริงจากใน viewer (#44, #45)

ผู้อ่านเจอจุดที่อยากทักตอนอ่านโค้ดใน viewer แต่ต้องสลับไปหน้า PR แล้วไล่หาบรรทัดเดิมเอง
ทั้งที่ viewer รู้ PR number / commit / บรรทัดอยู่แล้ว — ตั๋วนี้ปิดช่องว่างนั้นโดย
**ไม่สร้างที่เก็บโน้ตชุดที่สอง**: ทุกอย่างขึ้น GitHub จริง (agent รอบถัดไปจึงอ่านต่อได้ผ่าน gh)

- **browser ไม่แตะ GitHub เอง — server เรียก `gh api` ให้**: token อยู่กับ gh บนเครื่องผู้ใช้
  อยู่แล้ว การส่งมันเข้า browser คือการเพิ่มที่ให้รั่วโดยไม่ได้อะไรกลับมา · แลกมาด้วยการ
  **ผ่อน read-only guard** ของ `server/api.ts` ซึ่งเป็นการกลับคำครั้งเดียวของ v3 — ผ่อน
  **เฉพาะ path ที่ segment ที่สามเป็น `comments`** และเฉพาะ POST/PATCH/DELETE ที่เหลือยัง 405
  เหมือนเดิม (มีเทสต์ยืนยันว่า `POST /api/runs`, `PATCH …/file`, และ `PUT …/comments` ยังถูกปฏิเสธ)
- **เงื่อนไขก่อน merge (จากสเปก): Cloudflare Access ต้องเปิดใช้แล้ว** — endpoint เหล่านี้
  act ในนามบัญชี GitHub ของเจ้าของเครื่อง และ viewer ไม่มี auth layer ของตัวเองโดยตั้งใจ ·
  **ยังไม่ได้ยืนยันว่าเปิดแล้ว — เป็น gate ที่คนต้องตอบ ไม่ใช่สิ่งที่โค้ดตรวจได้**
- **Access ไม่ใช่ด่านเดียวที่ต้องมี: CSRF ก็ต้องกันด้วย** — Access กันคนที่ยิงมาที่ tunnel แต่ไม่
  กัน browser ของเจ้าของเครื่องเองที่ถูกหน้าเว็บอื่นสั่งให้ยิงมาที่ `127.0.0.1:5174` (พอร์ตคงที่
  + run id เดาได้ = `pr-<n>-<slug>`) · รูปที่ทะลุจริงคือ `<form enctype="text/plain">` ข้าม origin:
  ไม่ต้อง preflight และ CORS ไม่ช่วยเพราะ comment ถูกสร้างไปแล้วก่อนที่ browser จะบล็อกการอ่าน
  คำตอบ · กันสองชั้นที่ `assertSameSite()` ใน `server/api.ts` — `Sec-Fetch-Site` ต้องเป็น
  same-origin/none (header นี้หน้าเว็บตั้งเองไม่ได้ · client ที่ไม่ส่งมาใช้ `Origin` เทียบ host
  แทน ซึ่งเข้ากับ tunnel เพราะ cloudflared ส่ง Host เดิมต่อมา) **และ** content-type ต้องเป็น
  `application/json` · ตรวจ **ก่อนอ่าน body** ไม่ให้ request จากเว็บอื่นถูกประมวลผลแม้แต่ไบต์เดียว
- **"เทียบ diff ไม่ได้" ต้องไม่ถูกเหมาเป็น "บรรทัดอยู่นอก diff"** — `loadDiff()` คืน
  `status: 'unavailable'` + `hunks: []` เมื่อไม่มี `baseCommit` หรือยังไม่ `git fetch` base มา
  (สถานะปกติมากตอน review PR ของคนอื่นบน clone ใหม่) · รอบแรกดูแค่ `hunks` ผลคือ **review comment
  ใช้ไม่ได้ทั้งฟีเจอร์แบบเงียบ ๆ** และ toast ประกาศเหตุผลที่ผิด ("บรรทัด N ไม่อยู่ใน diff" ทั้งที่
  ไม่มีใครรู้) · ตอนนี้ response มี `fallback: { kind, reason }` แยกสองเคส และข้อความบอกผลเป็น
  ฟังก์ชันล้วน `commentResultMessage()` ที่มีเทสต์ — หลักคือ **ห้ามยืนยันสิ่งที่ระบบไม่รู้**
  (user story 11 ขอ "รู้ผลชัดเจน" ไม่ใช่ "ได้เหตุผลอะไรก็ได้")
- **ตัวรัน gh เป็น dependency ที่ inject ได้** (`createApiHandler({ gh })`) — seam ใหม่จุดเดียว
  ที่ตั๋วนี้เปิด · เทสต์ (`test/comments.test.ts`) จึงพิสูจน์สิ่งที่สำคัญจริงได้ครบ คือ
  **payload ที่ส่งถึง GitHub** (review comment ผูก `commit_id`, permalink ของ fallback,
  endpoint ที่ถูกตัวของ PATCH/DELETE) โดยไม่ต้องมี PR จริง — fixture ยังเป็น git repo จริง
  เพราะการตัดสิน "บรรทัดอยู่ใน diff ไหม" มาจาก git จริง
- **บรรทัดนอก diff ต้องทักได้ ไม่ใช่ถูกห้าม**: GitHub รับ review comment เฉพาะบรรทัดใน diff
  การบล็อกตรงนั้นเท่ากับบอกว่า "ทักไม่ได้" ทั้งที่คำถามส่วนใหญ่เกิดกับโค้ดเดิมที่ PR ไม่ได้แตะ
  (ซึ่งเป็นเหตุผลที่ viewer แสดงโค้ดรอบ ๆ ตั้งแต่แรก) — จึง fallback เป็น issue comment ที่มี
  permalink `blob/<sha>/<path>#L<n>` นำหน้า แล้ว **บอกผู้ส่งด้วย toast ว่ามันไปโผล่ที่ไหน**
- **ใช้ hunk ชุดเดียวกับที่ลงสี (`loadDiff`, `-U0`) ตัดสินว่าอยู่ใน diff ไหม** — แคบกว่าที่ GitHub
  ยอมรับจริง (GitHub นับบริบทรอบ hunk ด้วย) ซึ่งเป็นทิศที่ปลอดภัย: ทุกบรรทัดที่เราส่งเป็น review
  comment GitHub รับแน่ ๆ ส่วนที่ตกไปก็ยังทักได้ทางที่สอง — ไม่มีทางที่ผู้อ่านจะทักไม่ได้
- **comment เปิดจาก gutter, navigation เปิดจาก symbol** (ข้อกำหนดของสเปกเอง) — บนโค้ดมีเจ้าของ
  อยู่แล้วสองคน (เลือกข้อความ + กดค้างของ #43) การเอา comment ไปแย่งตำแหน่งเดียวกันแปลว่าทุกครั้ง
  ที่แตะโค้ดต้องมีใครแพ้ · แถบ comment เป็น `gutter()` ของ CodeMirror ที่มี `domEventHandlers`
  ของตัวเอง จึงไม่แตะ pointer handler ของ navigation เลย
- **gh ใช้ไม่ได้ = ไม่มีแถบให้กด** ไม่ใช่แถบที่กดแล้วส่งไม่ได้ (หลักข้อ 9) — เหตุผลถูกประกาศ
  ดัง ๆ ที่กล่องระดับ PR ท้ายหน้า run พร้อม error code และปุ่ม "ลองใหม่" · `gh auth status`
  ถูกตรวจครั้งเดียวแล้วจำ **เฉพาะฝั่งที่สำเร็จ** — คนที่เพิ่งไป `gh auth login` ต้องใช้ได้ทันที
  โดยไม่ต้องรีสตาร์ท viewer (บทเรียนเดียวกับ `knownCommits` ใน `git.ts`)
- **state ของ comment อยู่ที่ `RunLayout`** เหมือน reading panel — badge ในกล่องโค้ดกับกล่อง
  ท้ายหน้าใช้ชุดเดียวกัน และไม่ถูกดึงใหม่ทุกครั้งที่เปลี่ยน section · ดึงตอนเปิด + ปุ่ม refresh
  เท่านั้น (ไม่ poll ตามสเปก) · ผลของการส่ง/แก้ merge จาก **สิ่งที่ GitHub ตอบกลับ** ไม่ใช่ค่าที่เดาเอง
- **preview ใช้ `<Prose>` ตัวเดิมของ viewer** ไม่ใช่ renderer ตัวที่สอง — มันตอบคำถาม "โครงถูกไหม"
  ไม่ใช่ "สีตรงกับ GitHub เป๊ะไหม" ซึ่งเป็นคำถามที่ไม่มีใครถาม · กล่องเขียน **ไม่ล้างข้อความทิ้ง
  ตอนส่งไม่สำเร็จ** (พิมพ์ยาว ๆ แล้วหายไปกับ error คือความเสียหายที่ไม่มีทางกู้)
- **ลบยืนยันสองจังหวะในที่ ไม่ใช่ `window.confirm`** — บนมือถือผ่าน tunnel กล่องของเบราว์เซอร์
  เด้งคนละที่กับสิ่งที่กด และเราต้องบอกได้ด้วยว่ากำลังจะลบอันไหน
- **ปุ่มแก้/ลบโผล่เฉพาะ comment ของบัญชีที่ gh ใช้อยู่** (`gh api user` ถามครั้งเดียวพร้อมการตรวจ
  auth) — สิทธิ์จริงยังเป็น GitHub ที่ตัดสิน viewer แค่ส่ง error ต่อ · ไม่รู้ว่าใครใช้อยู่ =
  ไม่โชว์ปุ่ม (ปุ่มที่กดแล้วโดนปฏิเสธแย่กว่าไม่มีปุ่ม)
- **Test seam**: HTTP surface เดิม + fake gh runner · ฝั่งแอปมีฟังก์ชันล้วนก้อนเดียวที่ต้องมีเทสต์
  คือการจับคู่ comment เข้ากับ path+line (`src/lib/comments.ts` → `test/comments-map.test.ts`)
  เพราะผิดแล้ว *เงียบ*: badge ขึ้นผิดบรรทัดผู้อ่านไม่มีทางรู้ว่ามีคนทักไว้แล้ว แล้วก็ทักซ้ำ ·
  UI (แถบ gutter, กล่องเขียน, preview) ตรวจด้วยมือตาม convention เดิม — รอบนี้ตรวจบน browser จริง
  ด้วย run ตัวอย่าง pr-230 (อ่าน comment จริงของ PR, แถบ gutter ครบทุกการ์ด, กดแล้วกล่องเปิดที่
  `path:line` ถูกตัว, preview markdown, badge + รายการ comment ที่บรรทัด, ยืนยันลบสองจังหวะ,
  และเคส gh ตอบ 404 → กล่องเตือน + ไม่มีแถบให้กด) · **การส่ง/แก้/ลบ ขึ้น PR จริงยังไม่ถูกทดสอบ
  ด้วยมือ** — ต้องทำกับ PR ของตัวเองก่อนใช้จริง (ดูรายการท้าย PR)
