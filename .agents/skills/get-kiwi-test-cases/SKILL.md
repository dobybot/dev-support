...
You can use the existing script in `.agents/skills/generate-test-cases/kiwi/search_testcases.py` to get test cases.
...
Example:
```bash
uv run python .agents/skills/generate-test-cases/kiwi/search_testcases.py --id 1234
```
...
uv run python .agents/skills/generate-test-cases/kiwi/search_testcases.py --tag "DBT-100"

# Search by summary keywords
uv run python .agents/skills/generate-test-cases/kiwi/search_testcases.py --summary "eTax"

# Search by both
uv run python .agents/skills/generate-test-cases/kiwi/search_testcases.py --tag "DBT-100" --summary "upload"

# Filter by product or category PK
uv run python .agents/skills/generate-test-cases/kiwi/search_testcases.py --product 1 --category 5
```

Alternatively, use `lookup.py` with custom JSON filters if more complex queries are needed.
```bash
# Search by complex filter
uv run python .agents/skills/generate-test-cases/kiwi/lookup.py TestCase '{"category__product": 1, "category": 5}'
```
